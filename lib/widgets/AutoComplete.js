...
function TypeAhead (props) {
    if (!props.markedValue) {
        ...
        const suffix = (
            props.markedValue.title.indexOf(text) === 0 ?
...
class AutoCompleteBaseWidget extends Component {
    constructor (props) {
        ...
        this.state = {
            text: props.value && field.toFormattedString(props.value),
            isOpen: false,
            options: [],
            markAtIndex: undefined
        }
    ...
    componentWillReceiveProps (nextProps) {
        ...
        this.setState({
            text: nextProps.value && field.toFormattedString(nextProps.value),
