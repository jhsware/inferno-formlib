...
function renderRows ({ field, value, lang, disableI18n, namespace, inputName, itemKeys, validationErrors, customWidgets, onInput, onChange, onDelete, isMounted }) {
  ...
  return value.map((item, index) => {
    ...
    return (
      <ListRowContainer className="InfernoFormlib-DragItem" key={myNamespace.join('.')} data-drag-index={index} isFirstMount={!isMounted} value={value[index]} onDelete={() => onDelete(index)}>
