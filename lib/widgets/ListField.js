        ...
        return (
            <div className={"InfernoFormlib-ListFieldRow" + (this.props.className ? ' ' + this.props.className : '')} data-drag-index={this.props['data-drag-index']}>
                <div className="InfernoFormlib-DragHandle"
                    ...
                    onMouseUp={this.doMakeUndraggable}></div>
                {this.props.children}
            </div>
        )
    }
...
function renderRows ({ field, value, lang, namespace, inputName, itemKeys, validationErrors, customWidgets, onChange, onDelete, isMounted }) {
  ...
  return value.map((item, index) => {
    ...
    return (
      <ListFieldRow className="InfernoFormlib-DragItem" key={myNamespace.join('.')} data-drag-index={index} isFirstMount={!isMounted}>
