import { unstable_renderSubtreeIntoContainer } from "inferno-compat";
...
export function handleDragOver (e) {
    ...
    // We are now outside the container
    if (draggable && getDragContainer(draggable) !== this.currentDragContainer) return
...
export function handleDrop (e) {
    ...
    if (!draggable) return
    _clearDragMarkers(draggable, 'before', 'after')
    if (getDragContainer(draggable) !== this.currentDragContainer) return

    if (this.currentSource && this.currentTarget && this.currentTarget !== this.currentSource) {
